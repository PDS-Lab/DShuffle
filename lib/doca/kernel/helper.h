...
#define UNREACHABLE_CRIT DOCA_DPA_DEV_LOG_CRIT("%s:%d %s: unreachable\n", __FILE__, __LINE__, __func__);
...
#define LOG_INFO(fmt, ...) \
  DOCA_DPA_DEV_LOG_INFO("%s:%d %s: " fmt "\n", __FILE__, __LINE__, __func__ __VA_OPT__(, ) __VA_ARGS__)

#define LOG_V(v, fmt) LOG_INFO(#v ": " fmt, v);

#ifdef NDEBUG
#define LOG_DBG(fmt, ...) ((void)0)
#else
#define LOG_DBG(fmt, ...) ((void)0)
// DOCA_DPA_DEV_LOG_DBG("%s:%d %s: " fmt "\n", __FILE__, __LINE__, __func__ __VA_OPT__(, ) __VA_ARGS__)
#endif

// clang-format off
#define TRACE(a1)                  doca_dpa_dev_trace((uint64_t)a1,  0,  0,  0,  0)
#define TRACE2(a1, a2)             doca_dpa_dev_trace((uint64_t)a1, (uint64_t)a2,  0,  0,  0)
#define TRACE3(a1, a2, a3)         doca_dpa_dev_trace((uint64_t)a1, (uint64_t)a2, (uint64_t)a3,  0,  0)
#define TRACE4(a1, a2, a3, a4)     doca_dpa_dev_trace((uint64_t)a1, (uint64_t)a2, (uint64_t)a3, (uint64_t)a4,  0)
#define TRACE5(a1, a2, a3, a4, a5) doca_dpa_dev_trace((uint64_t)a1, (uint64_t)a2, (uint64_t)a3, (uint64_t)a4, (uint64_t)a5)
#define TRACE_FLUSH()              doca_dpa_dev_trace_flush()
// clang-format on
...
#else
#define PRINT_TICK(n) DOCA_DPA_DEV_LOG_INFO("%s:%d %s: " REP_N(n, "%lu") "\n", __FILE__, __LINE__, __func__ SEG_N(n))
#endif
